...
// this is not where this should be defined
...
void SCRF::nestedSCRF(mrcpp::ComplexFunction V_vac) {
    ...
    while (update >= this->conv_thrs && iter <= max_iter) {
        ...
        // compute new PB term
        if (this->kappa.norm() != -1.0) { computePBTerm(V_tot); }
